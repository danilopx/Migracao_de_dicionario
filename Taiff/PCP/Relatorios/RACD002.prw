...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function ListaQry(cAlias, cTitulo)
	Local bAddDados := ""
	...
	AADD( aButtons, {"HISTORIC", {|| U_PrevisaoIni( oGDLst:aCols[oGDLst:oBrowse:nAt][2] ) , nOpcA := 2, oDlg1:End() }, "Previsão de Início de Produção","Previsão",{|| .T.}} )     
	...
	oDlg1:bInit          := {|| EnchoiceBar(oDlg1, {|| _cLineBrow := oGDLst:aCols[oGDLst:oBrowse:nAt][nPosProduto], nOpcA := 1, oDlg1:End() }, {|| _cLineBrow := "", lNoFiltro	:= .F. , nOpcA := 0, oDlg1:End()},,@aButtons )}
	oDlg1:lCentered      := .T.
	...
	oGDLst:oBrowse:bLDblClick := {|z,x| _cLineBrow := oGDLst:aCols[oGDLst:oBrowse:nAt][nPosProduto], nOpcA := 1, oDlg1:End() } 

	nPosProduto:= aScan(aHeader,{|x| AllTrim(x[2])=="C2_PRODUTO"})
