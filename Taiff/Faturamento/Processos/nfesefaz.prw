...
//30-Set-2021 Ronald Piscioneri: adequacao para NT 2020.005 v1.20 (obrigatorio a partir de 04-Out-2021)
//06-Ago-2021 Ronald Piscioneri: maior parte das customizacoes foram transferidas para o PE01NFESEFAZ
...
//-------------------------------------------------
// Especifico TAIFF:
// Aglutinacao de Produtos para NFs de Transferencia
//-------------------------------------------------
Local aAglut		:= {}

//-----------------------------------------------------------------------------------------------------
// Especifico TAIFF:
// Clientes que quebram a regra tratamento para a lei da Portaria Suframa nº 275/2009 para Pis e Cofins
//-----------------------------------------------------------------------------------------------------
Local cCliLeiZFM 	:= SuperGetMV("TF_LEIZFM",,"")
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				If !SF2->F2_TIPO $ "DB" 
					...
					// Especifico TAIFF:
					// Clientes que quebram a regra tratamento para a lei da Portaria Suframa nº 275/2009 para Pis e Cofins
					// Desativa a tag vDesc para clientes especificos (ex. Carrefour Manaus)
					//-----------------------------------------------------------------------------------------------------
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						*/
						...
						//-------------------------------------------------
						// Especifico TAIFF:
						// A ultima posicao do aProd deve ser o D2_COD,
						// Pois sera validado no PE01NFESEFAZ
						//-------------------------------------------------
						aadd(aProd,	{Len(aProd)+1,;
							...
							(cAliasSD2)->D2_COD; // Especifico TAIFF - codigo do produto a ser validado no PE01NFESEFAZ
							})
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				*/				
				...
				//-------------------------------------------------
				// Especifico TAIFF:
				// A ultima posicao do aProd deve ser o D1_COD,
				// Pois sera validado no PE01NFESEFAZ
				//-------------------------------------------------
				aadd(aProd,	{Len(aProd)+1,;  
					cCodProd,;
					...
					SF4->F4_ART274,; //aprod[48]
					0,;  //aprod[49]   nValLeite
					(cAliasSD1)->D1_COD; //Especifico TAIFF - codigo do produto a ser validado no PE01NFESEFAZ
					})
...
IF lPe01Nfe     
	...
	// Aglutinacao de Produtos para NFs de Transferencia
	//-------------------------------------------------
	aAglut := {aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aVeicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ}

	aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aEspVol,aNfVinc,aDetPag,aObsCont,aProcRef,aMed,aLote,aPedCom,cIndPres,cIndIntermed,aAglut}
	...
	If ( Len(aParam) >= 5 )
		...
		aPedCom		:= aParam[21]
		cIndPres	:= aParam[22]
		cIndIntermed:= aParam[23]

		//-------------------------------------------------
		// Especifico TAIFF:
		// Aglutinacao de Produtos para NFs de Transferencia
		//-------------------------------------------------
		aAglut		:= aParam[Len(aParam)]
		aICMS		:= aAglut[1]
		aICMSST		:= aAglut[2]
		aIPI		:= aAglut[3]
		aPIS		:= aAglut[4]
			...
			If nBaseIcm > 0 .and. nValICM > 0	.and. nAlqICM > 0
			    ...
			    If cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "SP"
					If Empty(At("Artigo 274 do RICMS", cMensFis))
						...
						//-------------------------------------------------
						// Especifico TAIFF:
						// Mensagem para Artigo 274 RICMS - SP
						//-------------------------------------------------
						cMensFis += "Imposto Recolhido por Substituição - Artigo 274 do RICMS (Lei 6.374/89, art.67,Paragrafo 1o., e Ajuste SINIEF-4/93',cláusa terceira, na redação do Ajuste SINIEF-1/94) "
						cMensFis += "Cod.Produto: "+Alltrim(ConvType(aProd[02]))+" "
						cMensFis += "Valor da Base de ST: R$ "+AllTrim(str(nBaseIcm,15,2))+" "
						cMensFis += "Valor de ICMS ST: R$ "+AllTrim(str(nValICM,15,2)) + " "
						cMensFis += "Valor da parcela do imposto retido: R$ " + Alltrim(IiF((nBaseIcm*(18/100))-(aProd[10]*(18/100))>0,Alltrim(str((nBaseIcm*(18/100))-(aProd[10]*(18/100)),15,2)),"0.00"))+" "
