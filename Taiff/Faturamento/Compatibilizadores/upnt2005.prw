...
/*/
//--------------------------------------------------------------------
User Function UPNT2005( cEmpAmb, cFilAmb )

Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
Local   cTitulo   := "ATUALIZAÇÃO DE CAMPOS PARA NT 2020.005 (29-Set-2021)"
Local   cDesc1    := "Esta rotina criará os campos abaixo, na tabela CD2:"
Local   cDesc2    := "CD2_VFCPDI"
Local   cDesc3    := "CD2_VFCPEF"
Local   cDesc4    := "CD2_PSCFST"
Local   cDesc5    := "Obs.: estes campos sao utilizados no NFeSefaz."
Local   cDesc6    := ""
Local   cDesc7    := ""
Local   cMsg      := ""
Local   lOk       := .F.
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPNT2005" )
				Else
					MsgStop( "Atualização não Realizada.", "UPNT2005" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'CD2'																	, .T. }, ; //X3_ARQUIVO
	{ '39'																	, .T. }, ; //X3_ORDEM
	{ 'CD2_PSCFST'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Ag Pis/CofST'														, .T. }, ; //X3_TITULO
	{ 'Ag Pis/CofST'														, .T. }, ; //X3_TITSPA
	{ 'Agg Pis/CofS'														, .T. }, ; //X3_TITENG
	{ 'Agrega Pis/CofST'													, .T. }, ; //X3_DESCRIC
	{ 'Agrega Pis/CofST'													, .T. }, ; //X3_DESCSPA
	{ 'Aggregate Pis/CofST'													, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'CD2'																	, .T. }, ; //X3_ARQUIVO
	{ '40'																	, .T. }, ; //X3_ORDEM
	{ 'CD2_VFCPEF'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 14																	, .T. }, ; //X3_TAMANHO
	{ 2																		, .T. }, ; //X3_DECIMAL
	{ 'Vlr FCP Efet'														, .T. }, ; //X3_TITULO
	{ 'Val. FCP Efe'														, .T. }, ; //X3_TITSPA
	{ 'Eff. FCP Val'														, .T. }, ; //X3_TITENG
	{ 'Valor de FECP Efetivo'												, .T. }, ; //X3_DESCRIC
	{ 'Valor de FECP efectivo'												, .T. }, ; //X3_DESCSPA
	{ 'Effective FECP value'												, .T. }, ; //X3_DESCENG
...
aAdd( aSX3, { ;
	{ 'CD2'																	, .T. }, ; //X3_ARQUIVO
	{ '41'																	, .T. }, ; //X3_ORDEM
	{ 'CD2_VFCPDI'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 14																	, .T. }, ; //X3_TAMANHO
	{ 2																		, .T. }, ; //X3_DECIMAL
	{ 'Vlr FECP Dif'														, .T. }, ; //X3_TITULO
	{ 'Val. FECP Di'														, .T. }, ; //X3_TITSPA
	{ 'Vl FECP Dif'															, .T. }, ; //X3_TITENG
	{ 'Valor de FECP Diferido'												, .T. }, ; //X3_DESCRIC
	{ 'Valor de FECP Diferido'												, .T. }, ; //X3_DESCSPA
	{ 'Deferred FECP Value'													, .T. }, ; //X3_DESCENG
	{ '@E 99,999,999,999.99'												, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPNT2005" ) ) ) ;
