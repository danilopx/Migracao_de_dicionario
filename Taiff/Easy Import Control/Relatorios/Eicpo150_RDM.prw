...
//Caso deseje usar o relatório personalizado descompile o fonte e apague a chamada user function eicpo150() e utilize a po150rdm() no bloco abaixo.
User Function EICPO150()
Return EICPO150()


/*User Function PO150RDM()

//" RDMake comentado e substituído por um novo modelo padrão.
//Relatório comentado para ser desviado e ser impresso o novo modelo do EICPO150, para executar o relatório no modelo antigo descomente o fonte
//e retire a chamada do novo relatório padrão no retorno da chamada e em seguida compile esse rdmake"
...
#COMMAND    TRACO_REDUZIDO                 => oSend(oPrn,"Line", Linha  ,1511, Linha  , 2300 ) ;
...
cPaisImpor := Alltrim(Posicione("SYA",1,xFilial("SYA")+SYT->YT_PAIS,"YA_DESCR"))
...
c2EndSYT := c2EndSYT + IF( !EMPTY(cPaisImpor), cPaisImpor  +' - ', "" )
...
IF nIdioma == INGLES
   cTerms := MSMM(SY6->Y6_DESC_I,AVSX3("Y6_VM_DESI",3))
ELSE
   cTerms := MSMM(SY6->Y6_DESC_P,AVSX3("Y6_VM_DESP",3))
ENDIF
cTerms := STRTRAN(cTerms, CHR(13)+CHR(10), " ")
...
oSend( oPrn, "Say",  Linha, 630, SA2->A2_NREDUZ + Space(2) + Alltrim(IF(EICLOJA(), LITERAL_STORE + Alltrim(SA2->A2_LOJA),"")) , aFontes:TIMES_NEW_ROMAN_12 )
Linha := Linha+50
...
If lNewProforma .and. EasyGParam("MV_AVG0186",,.F.)
   EYZ->(DbSetOrder(2))
...
If Len(Alltrim(cNomeFantasia)) > 50
	...
	oSend( oPrn, "Say",  Linha,1450, LEFT(AllTrim(SA2->A2_NREDUZ),len(cNomeFantasia)/2) + Space(2) + IF(EICLOJA(), LITERAL_STORE + Alltrim(SA2->A2_LOJA),"") ,aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 ) //FDR - 06/01/12
		Linha := Linha + 30
	oSend( oPrn, "Say",  Linha,1300, RIGHT(AllTrim(SA2->A2_NREDUZ),len(cNomeFantasia)/2) + Space(2) ,aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1) 
		Linha := Linha - 30
else
    oSend( oPrn, "Say",  Linha,1450, AllTrim(SA2->A2_NREDUZ) + Space(2) + IF(EICLOJA(), LITERAL_STORE + Alltrim(SA2->A2_LOJA),"") ,aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 )
endIF
...
While !Empty(cRemarks)
   ...
   If nLinRemark == 10 .Or. Linha >= 3100
      TRACO_NORMAL
      ENCERRA_PAGINA
      COMECA_PAGINA(.F.)
      TRACO_NORMAL
...
oSend( oPrn, "Say",  Linha,1570, STR0044, aFontes:COURIER_08_NEGRITO )
nTLinha := Linha 
...
IF !Empty(x)
   cData := SUBSTR(DTOC(x)  ,1,2)+ " " + IF( nIdioma == INGLES, SUBSTR(CMONTH(x),1,3) , SUBSTR(Nome_Mes(MONTH(x)),1,3) ) + " " + LEFT(DTOS(x)  ,4)
EndIf
